...
void decrypt_file(char *encrypted_filename, char *output_filename, uint16_t key, uint8_t num_rounds) {
	...
	while (!feof(input_file)) {
		...
		if (num_read == 3) {
			uint16_t right_bits = 4095;
			uint16_t left_bits = 4095;

			// Splits bits into right and left
			right_bits = right_bits & input_val;
			left_bits = left_bits & (input_val >> 12);
			
			// Decrypt right and left bits
			right_bits = decrypt(right_bits, keys, num_rounds);
			left_bits = decrypt(left_bits, keys, num_rounds);	
			
			// Swap right and left bits
			uint32_t final_value = left_bits;
			final_value = final_value << 12;
			final_value = final_value | right_bits;
			
			// If padding exists, then it will be removed
			if (remaining_reads != 0) {
				fwrite(&final_value, 1, 3, output_file);
			} else {
				fwrite(&final_value, 1, 3 - padding, output_file);
			}
